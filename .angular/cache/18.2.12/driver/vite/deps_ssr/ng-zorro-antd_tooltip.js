...
var NzTooltipBaseDirective = class _NzTooltipBaseDirective {
  ...
  createComponent() {
    ...
    this.initProperties();
    const ngVisibleChange$ = this.component.nzVisibleChange.pipe((0, import_operators.distinctUntilChanged)());
    ngVisibleChange$.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((visible) => {
      this.internalVisible = visible;
      this.visibleChange.emit(visible);
    });
    ngVisibleChange$.pipe((0, import_operators.filter)((visible) => visible), (0, import_operators.delay)(0, import_rxjs.asapScheduler), (0, import_operators.filter)(() => Boolean(this.component?.overlay?.overlayRef)), (0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
      this.component?.updatePosition();
    });
  }
  registerTriggers() {
...
var NzTooltipBaseComponent = class _NzTooltipBaseComponent {
  constructor() {
    ...
    this.nzVisibleChange = new import_rxjs.Subject();
    this._visible = false;
    this._trigger = "hover";
