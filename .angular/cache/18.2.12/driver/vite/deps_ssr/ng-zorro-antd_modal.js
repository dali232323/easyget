...
var NzSafeNullPipe = class _NzSafeNullPipe {
  ...
  static {
    this.ɵfac = function NzSafeNullPipe_Factory(__ngFactoryType__) {
...
var BaseModalContainerComponent = class _BaseModalContainerComponent extends BasePortalOutlet {
  ...
  constructor() {
    ...
    this.nzConfigService.getConfigChangeEventForComponent(NZ_CONFIG_MODULE_NAME).pipe((0, import_operators2.takeUntil)(this.destroy$)).subscribe(() => {
  ...
  setupMouseListeners(modalContainer) {
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs2.fromEvent)(this.host.nativeElement, "mouseup").pipe((0, import_operators2.takeUntil)(this.destroy$)).subscribe(() => {
        if (this.mouseDown) {
          setTimeout(() => {
            this.mouseDown = false;
          });
      ...
      (0, import_rxjs2.fromEvent)(modalContainer.nativeElement, "mousedown").pipe((0, import_operators2.takeUntil)(this.destroy$)).subscribe(() => {
        this.mouseDown = true;
      });
    });
  }
...
var NzModalRef = class {
  constructor(overlayRef, config, containerInstance) {
    ...
    this.state = 0;
    this.afterClose = new import_rxjs2.Subject();
    this.afterOpen = new import_rxjs2.Subject();
    this.destroy$ = new import_rxjs2.Subject();
    containerInstance.animationStateChanged.pipe((0, import_operators2.filter)((event) => event.phaseName === "done" && event.toState === "enter"), (0, import_operators2.take)(1)).subscribe(() => {
      this.afterOpen.next();
    ...
    containerInstance.animationStateChanged.pipe((0, import_operators2.filter)((event) => event.phaseName === "done" && event.toState === "exit"), (0, import_operators2.take)(1)).subscribe(() => {
      clearTimeout(this.closeTimeout);
      this._finishDialogClose();
    });
    containerInstance.containerClick.pipe((0, import_operators2.takeUntil)(this.destroy$)).subscribe(() => {
    ...
    overlayRef.keydownEvents().pipe((0, import_operators2.filter)((event) => this.config.nzKeyboard && !this.config.nzCancelLoading && !this.config.nzOkLoading && event.keyCode === ESCAPE && !hasModifierKey(event))).subscribe((event) => {
      event.preventDefault();
      this.trigger(
        "cancel"
        /* NzTriggerAction.CANCEL */
    ...
    containerInstance.cancelTriggered.pipe((0, import_operators2.takeUntil)(this.destroy$)).subscribe(() => this.trigger(
      "cancel"
      /* NzTriggerAction.CANCEL */
    ));
    containerInstance.okTriggered.pipe((0, import_operators2.takeUntil)(this.destroy$)).subscribe(() => this.trigger(
      "ok"
      /* NzTriggerAction.OK */
    ));
  ...
  close(result) {
    ...
    this.result = result;
    this.containerInstance.animationStateChanged.pipe((0, import_operators2.filter)((event) => event.phaseName === "start"), (0, import_operators2.take)(1)).subscribe((event) => {
...
var NzModalService = class _NzModalService {
  ...
  constructor(overlay, injector, nzConfigService, directionality) {
    ...
    this.afterAllClosedAtThisLevel = new import_rxjs2.Subject();
    this.afterAllClose = (0, import_rxjs2.defer)(() => this.openModals.length ? this._afterAllClosed : this._afterAllClosed.pipe((0, import_operators2.startWith)(void 0)));
...
var NzModalComponent = class _NzModalComponent {
  ...
  open() {
    ...
    if (!this.modalRef) {
      ...
      this.modalRef.afterClose.asObservable().pipe((0, import_operators2.takeUntil)(this.destroy$)).subscribe(() => {
        this.close();
      });
    }
  }
